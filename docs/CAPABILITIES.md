...
| 独立 Critic、结构化问题与确定性 Ranker | fake/live Critic 已实现 | 离线测试通过；兼容端点真实 smoke 通过 | `backend/app/graphs/critic_subgraph.py`、`backend/app/services/critic_service.py` |
...
| React 工作台、Raw 审片、Prompt 历史、Critic、时间线 | 已实现 | 组件测试、生产构建与应用浏览器验收通过 | `frontend/src/app/App.tsx`、`frontend/src/features/search/PromptHistory.tsx` |
