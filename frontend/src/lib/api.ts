...
function normalizeAsset(value: unknown): AssetRef | undefined {
  ...
  return {
    ...
    content_url: stringValue(value.content_url) || undefined,
    url: stringValue(value.url) || undefined,
    asset_url: stringValue(value.asset_url) || undefined,
  };
}
...
function imageUrlFromCandidate(candidate: JsonObject): string {
  const direct = stringValue(
    candidate.image_url,
    stringValue(candidate.asset_url, stringValue(candidate.protected_image_url)),
  );
...
function normalizeCandidate(
  ...
  return {
    ...
    image_url: imageUrlFromCandidate(value),
    raw_image_url: stringValue(value.raw_image_url) || undefined,
...
export async function resumeSearch(
  ...
  await request(`/searches/${encodeURIComponent(searchId)}/resume`, {
    ...
    body: JSON.stringify({ action, updated_user_intent: null }),
  });
}
